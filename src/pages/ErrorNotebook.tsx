...
import { Calculator, BookOpen, PenLine, PartyPopper } from 'lucide-react'
...
export default function ErrorNotebook() {
  ...
  const maxCount = errorNotebook.topicBreakdown.length > 0 ? Math.max(...errorNotebook.topicBreakdown.map((t) => t.count)) : 1
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {errorNotebook.topicBreakdown.length > 0 && (
      ...
      <div id="review-list" className="flex flex-col gap-8">
        {total === 0 ? (
          <div className="flex flex-col items-center gap-2 rounded-xl2 bg-essay/15 p-10 text-center">
            <PartyPopper className="text-essay-ink" size={28} />
            <p className="font-semibold text-essay-ink">Nenhum erro por aqui ainda</p>
            <p className="text-sm text-ink-soft">
              Assim que você errar alguma questão, ela aparece aqui para você revisar depois.
            </p>
          </div>
        ) : (
